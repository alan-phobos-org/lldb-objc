...
def validate_basic_watch():
    ...
    def validator(output):
        if "breakpoint" in output.lower() or "Breakpoint" in output:
            ...
            return (
                True,
                "Breakpoint created (auto-continue flag may not be visible in list)",
            )
        elif "owatch" in output or "watching" in output.lower():
            return True, "Watch command executed"
        return False, (
            f"No breakpoint created\n"
...
def validate_instance_method_watch():
    ...
    def validator(output):
        if "NSString" in output or "breakpoint" in output.lower() or "length" in output:
            ...
            return False, (
                f"Error watching instance method\n"
                f"    Expected: Watch on -[NSString length]\n"
                f"    Actual: Error encountered\n"
                f"    Output preview: {output[:200]}"
            )
        return False, (
            f"Unexpected output for instance method watch\n"
...
def validate_list_command():
    ...
    def validator(output):
        ...
        return False, (
            f"Unexpected output for list command\n"
...
def validate_clear_command():
    ...
    def validator(output):
        ...
        return False, (
            f"Unexpected output for clear command\n"
...
def get_test_specs():
    ...
    return [
        ...
        (
            "Multiple watches",
            [
                "owatch -[NSString description]",
                "owatch +[NSDate date]",
                "owatch -[NSArray count]",
                "breakpoint list",
            ],
