...
def validate_ivars_and_properties():
    ...
    def validator(output):
        ...
        if has_ivars and has_props:
            ...
            return False, (
                f"Only properties shown, missing ivars\n"
                f"    Expected: Both 'Instance Variables' and 'Properties' sections\n"
                f"    Actual: Only 'Properties' found\n"
                f"    Output preview: {output[:300]}"
            )
        return False, (
            f"Neither section found\n"
            f"    Expected: Both 'Instance Variables' and 'Properties' sections\n"
            f"    Actual: Neither found\n"
            f"    Output preview: {output[:300]}"
        )
...
def validate_class_no_ivars():
    ...
    def validator(output):
        ...
        return False, (
            f"Unexpected output for class with no ivars\n"
            f"    Expected: 'Instance Variables' section or 'none'\n"
            f"    Actual: Neither found\n"
            f"    Output preview: {output[:300]}"
        )
...
def validate_class_no_properties():
    ...
    def validator(output):
        ...
        return False, (
            f"Unexpected output for class with no properties\n"
...
def get_test_specs():
    ...
    return [
        # Ivars tests
        ("--ivars basic: NSObject", ["ocls --ivars NSObject"], validate_ivars_basic()),
        (
            "--ivars type display",
            ["ocls --ivars NSString"],
        ...
        # Edge cases
        ("Class with no ivars", ["ocls --ivars NSProxy"], validate_class_no_ivars()),
        (
