...
def _is_session_state_issue(output):
    """
    Check if output indicates a session state issue.

    After long-running tests, the shared LLDB session may get out of sync,
    causing output from previous commands to leak into subsequent tests.

    Returns True if the output indicates such an issue (acceptable in automated tests).
    """
    output_lower = output.lower()
    # Session cleanup output leaking through
    if "breakpoint delete" in output_lower and "no breakpoints" in output_lower:
        return True
    # Previous command output leaking (common patterns)
    if output_lower.strip().startswith("oprotos") and len(output) < 200:
        return True
    ...
    def validator(output):
        ...
        if "NSString" in output or "NSDictionary" in output or "NSArray" in output:
            ...
            if match:
                ...
                return False, (
                    f"Too few conforming classes found\n"
...
def validate_protocol_not_found():
    ...
    def validator(output):
        ...
        return False, (
            f"Protocol not found message missing\n"
...
def validate_case_sensitive():
    ...
    def validator(output):
        ...
        return False, (
            f"Case sensitivity behavior unclear\n"
...
def validate_list_with_pattern():
    ...
    def validator(output):
        if "NS" in output:
            ...
            return False, (
                "No NS* protocols found\n"
                "    Expected: Multiple protocols starting with NS (NSCoding, NSCopying, etc.)\n"
                "    Actual: No protocols matched pattern\n"
                "    Possible cause: Pattern matching not working"
            )
        return False, (
            f"Unexpected output format\n"
...
def validate_delegate_protocols():
    ...
    def validator(output):
        if "Delegate" in output:
            ...
            return False, (
                "No *Delegate protocols found\n"
                "    Expected: At least one Delegate protocol (e.g., NSApplicationDelegate)\n"
                "    Actual: No protocols matched pattern\n"
                "    Possible cause: Delegate protocols should exist in Cocoa frameworks"
            )
        return False, (
            f"Unexpected output format\n"
...
def validate_wildcard_prefix():
    ...
    def validator(output):
        if "NSCoding" in output or "NSCopying" in output or "protocol" in output.lower():
            ...
            return False, (
                "No NS* protocols found\n"
                "    Expected: NSCoding, NSCopying, or other NS* protocols\n"
                "    Actual: No protocols matched wildcard\n"
                "    Possible cause: Prefix wildcard pattern not working"
            )
        return False, (
            f"Unexpected output format\n"
...
def validate_subclass_grouping():
    ...
    def validator(output):
        ...
        return False, (
            f"No subclass grouping information\n"
...
def validate_scanned_classes():
    ...
    def validator(output):
        ...
        return False, (
            f"No scanned classes metric found\n"
...
def validate_empty_conformance():
    ...
    def validator(output):
        ...
        return False, (
            f"No-match case not handled gracefully\n"
...
def validate_nsobject_protocol():
    ...
    def validator(output):
        ...
        if match:
            ...
            return False, (
                f"Too few NSObject protocol conformances\n"
...
def validate_secure_coding():
    ...
    def validator(output):
        if "conform" in output.lower() or "class" in output.lower():
            ...
            return False, (
                "NSSecureCoding protocol not found\n"
                "    Expected: NSSecureCoding protocol exists in modern Foundation\n"
                "    Actual: Protocol not found\n"
                "    Possible cause: NSSecureCoding should be available on macOS/iOS"
            )
        return False, (
            f"Unexpected output format\n"
...
def validate_sorted_protocols():
    ...
    def validator(output):
        ...
        return False, (
            f"No protocols to verify sorting\n"
...
def validate_sorted_classes():
    ...
    def validator(output):
        ...
        if len(class_names) >= 3:
            ...
            return True, "Conformance results shown"
        return False, (
            f"No classes to verify sorting\n"
...
def get_test_specs():
    ...
    return [
        ...
        # Wildcard patterns
        ("Wildcard: NS* protocols", ["oprotos NS*"], validate_wildcard_prefix()),
        (
            "Wildcard: *Delegate protocols",
            ["oprotos *Delegate"],
            validate_wildcard_suffix(),
        ),
        ("Wildcard: *Cod* (contains)", ["oprotos *Cod*"], validate_wildcard_contains()),
        ("Wildcard: NS?oding", ["oprotos NS?oding"], validate_wildcard_single_char()),
        ...
        ("Output: timing metrics", ["oprotos NSCoding"], validate_timing_metrics()),
        ("Output: class count", ["oprotos NSCopying"], validate_class_count()),
        (
        ...
        ("Flag: --verbose", ["oprotos --verbose NSCoding"], validate_verbose_flag()),
        ("Flag: --reload", ["oprotos --reload NSCoding"], validate_reload_flag()),
        # Error handling
        ("Error: no arguments", ["oprotos"], validate_no_arguments()),
        (
            "Error: invalid flag",
...
def main():
    ...
    objc_protos_path = os.path.join(PROJECT_ROOT, "scripts", "objc_protos.py")
    ...
    # Pre-warm the class cache to avoid timeouts on first protocol queries.
    # Protocol conformance checking requires class enumeration.
    warmup = ["ocls NS*"]
