...
def validate_single_match():
    ...
    def validator(output):
        ...
        return False, (
            f"No hierarchy display found for single match\n"
...
def validate_inheritance_chain():
    ...
    def validator(output):
        ...
        if has_mutable_string and has_string and has_object and has_arrows:
            return (
                True,
                "Complete inheritance chain shown: NSMutableString → NSString → NSObject",
            )
        elif has_mutable_string and "→" in output:
            return True, "Partial inheritance chain shown"
        arrow_count = output.count("→")
        return False, (
            f"Inheritance chain incomplete\n"
            f"    Expected: NSMutableString → NSString → NSObject (3 levels, 2+ arrows)\n"
            f"    Actual: Mutable={has_mutable_string}, String={has_string}, "
            f"Object={has_object}, Arrows={arrow_count}\n"
            f"    Output preview: {output[:200]}"
        )
...
def validate_few_matches():
    ...
    def validator(output):
        ...
        if match:
            ...
            if 2 <= count <= 20:
                if has_hierarchy:
                    return (
                        True,
                        f"Compact hierarchy shown for {count} matches (2-20 range)",
                    )
                return False, (
                    f"No hierarchy for {count} matches in 2-20 range\n"
        ...
        return False, (
            f"Expected compact hierarchy for few matches\n"
    ...
    def validator(output):
        ...
        if match:
            ...
            if 2 <= count <= 20:
                ...
                lines = output.split("\n")
                hierarchy_lines = [line for line in lines if "→" in line and "total" not in line.lower()]

                if len(hierarchy_lines) >= count:
                    return (
                        True,
                        f"All {count} classes show hierarchy ({len(hierarchy_lines)} hierarchy lines)",
                    )
                elif len(hierarchy_lines) > 0:
                    return (
                        True,
                        f"{len(hierarchy_lines)}/{count} classes show hierarchy",
                    )
                return False, (
                    f"No hierarchy lines found for {count} matches\n"
    ...
    def validator(output):
        ...
        if match:
            ...
            if count > 20:
                ...
                lines = output.split("\n")
                hierarchy_lines = [line for line in lines if line.strip().startswith("NS") and "→" in line]
                ...
                if len(hierarchy_lines) == 0:
                    ...
                    return (
                        True,
                        f"Minimal hierarchy for {count} matches ({len(hierarchy_lines)} with arrows)",
                    )
                return False, (
                    f"Too many hierarchy lines ({len(hierarchy_lines)}) for {count} matches\n"
            ...
            return False, (
                f"Expected >20 matches, got {count}\n"
...
def validate_root_class():
    ...
    def validator(output):
        ...
        return False, (
            f"NSObject not found in output\n"
...
def get_test_specs():
    ...
    return [
        # Single match tests
        ("Single Match (NSString)", ["ocls NSString"], validate_single_match()),
        (
        ...
        ("Few Matches (NSMutable*)", ["ocls NSMutable*"], validate_few_matches()),
        (
        ...
        ("Root class: NSObject", ["ocls NSObject"], validate_root_class()),
    ]
...
def main():
    ...
    # Pre-warm the class cache with a wildcard query to avoid timeouts
