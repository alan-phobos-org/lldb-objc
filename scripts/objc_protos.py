...
def get_all_protocols(frame: lldb.SBFrame, pattern: Optional[str] = None) -> Tuple[List[str], Dict[str, Any]]:
    """
    ...
    timing = {"total": 0, "expression_count": 0, "memory_read_count": 0}
...
def check_class_conforms_to_protocol(frame: lldb.SBFrame, class_ptr: int, protocol_ptr: int) -> bool:
    """
...
"""
        for i, (class_name, class_ptr) in enumerate(batch):
            batch_expr += (
                f"    results[{i}] = (unsigned char)"
                f"class_conformsToProtocol((Class)0x{class_ptr:x}, "
                f"(void *)0x{protocol_ptr:x});\n"
            )
...
def group_classes_by_inheritance(
    frame: lldb.SBFrame, conforming_classes: List[Tuple[str, bool]]
